...
func typeObject(t *testing.T, obj CustomJSONObject) string {
	l := obj.getLength()
	m := make([]byte, l)
	obj.writeToBytes(m)
	return string(m)
...
func getType(obj CustomJSONObject) string {
	switch obj.(type) {
	case JSONDictionary:
		return "Dictionary"
	case JSONNull:
...
func checkResult(t *testing.T, obj CustomJSONObject, waitingResult string) {
	s := typeObject(t, obj)
	if s != waitingResult {
...
func TestParsingObjects(t *testing.T) {
	...
	for i := 0; i < len(mustPassObjects); i++ {
		...
		//
		//	fmt.Println(JSONObjectToString(obj))
		src := JSONObjectToString(obj)
		dest := JSONObjectToString(obj.Copy())
		_, isDict := obj.(JSONDictionary)
		if src != dest && !isDict {
